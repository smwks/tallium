...
renders `flux:sidebar.brand`. Name comes from `config('app.name')` rather than the upstream
`:name="config('app.name', 'Laravel')"` binding:
...
> Upstream already made the brand name dynamic (`:name="config('app.name', 'Laravel')"`) inside
> the `@if`/`@else` toggle. TALLium's change is orthogonal -- it collapses the toggle itself, so
> the dynamic-name behavior carries over structurally rather than needing to be reintroduced.
...
**Note:** Upstream also ships `chiselSkipsNode()` / `chiselRemoveNpmPackages()` helpers (used by
the passkeys toggle to respect `install:features --no-node`). These don't reference
`routes/settings.php`, so the `sed` above leaves them untouched — no TALLium-specific changes
needed there.
